...
const rules = {
  name: [
    { required: true, message: 'Vui lòng nhập tên sản phẩm', trigger: ['blur', 'input'] },
  ],
  slug: [
...
async function handleSave() {
  try {
    ...
    const errorMessage = isEdit.value ? 'Cập nhật sản phẩm thất bại' : 'Thêm sản phẩm thất bại'
        ...
        <n-grid cols="3" x-gap="16" y-gap="16">
          <n-grid-item span="3">
            <n-form-item label="Tên sản phẩm" path="name">
              <n-input v-model:value="productForm.name" placeholder="Nhập tên sản phẩm" />
            </n-form-item>
          </n-grid-item>
          ...
          <n-grid-item span="1">
            <n-form-item label="Giá" path="price">
              <n-input-number v-model:value="productForm.price" :show-button="false" :parse="parse" :format="format" placeholder="Nhập giá sản phẩm" class="w-full" />
            </n-form-item>
          </n-grid-item>
        ...
        <n-collapse accordion class="custom-collapse-light-blue grow rd-5 my-15" arrow-placement="right">
          <n-collapse-item class="custom-collapse-header-light-blue" title="Thông tin Meta" name="meta">
            <n-form-item label="Meta Title">
              <n-input v-model:value="productForm.meta_title" placeholder="Nhập meta title" />
            </n-form-item>
            <n-form-item label="Meta Keywords">
              <n-input v-model:value="productForm.meta_keywords" placeholder="Nhập meta keywords" />
            </n-form-item>
            <n-form-item label="Meta Description">
              <n-input v-model:value="productForm.meta_description" type="textarea" placeholder="Nhập meta description" />
            </n-form-item>
            <n-form-item label="Meta Image">
              <n-upload
                v-model:file-list="productForm.meta_image"
                :max="1"
