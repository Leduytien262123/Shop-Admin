  ...
  full_name: [
    {
      ...
      validator(rule, value) {
        ...
        if (!name) return new Error("Vui lòng nhập tên nhân sự");
        if (name.length < 10)
          return new Error("Tên nhân sự phải có ít nhất 10 ký tự");
        if (name.length > 100)
          return new Error("Tên nhân sự không được quá 100 ký tự");
        return true;
      },
      trigger: ["blur", "input"],
    },
...
<template>
  <CommonPage>
    ...
    <n-card :title="isEdit ? 'Sửa nhân sự' : 'Thêm nhân sự'">
      <n-form :model="userForm" :rules="rules" ref="formRef">
        <n-grid cols="3" x-gap="16" y-gap="16">
          <n-grid-item span="1">
            <n-form-item label="Tên nhân sự" path="full_name">
              <NaiveInput
