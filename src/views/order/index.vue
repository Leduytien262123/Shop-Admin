          ...
          <n-grid cols="3" x-gap="12" y-gap="12">
            <n-grid-item span="1">
              <n-form-item label="Mã đơn hàng">
                <n-input
                  ...
                  @keyup.enter="throttledLoadOrders"
                />
              </n-form-item>
            </n-grid-item>  
            ...
            <n-grid-item span="1">
              <n-form-item label="Loại đơn hàng">
                <n-select v-model:value="searchQuery.type_order" @update:value="searchByQuery" :options="typeOrders" filterable placeholder="Tất cả loại đơn" clearable />
              </n-form-item>
            </n-grid-item>

            <n-grid-item span="1">
              <n-form-item label="Trạng thái xử lý">
                <n-select v-model:value="searchQuery.status" @update:value="searchByQuery" filterable placeholder="Tất cả trạng thái" clearable :options="optionsStatus" />
              </n-form-item>
            </n-grid-item>  

            <n-grid-item span="1">
              <n-form-item label="Trạng thái thanh toán">
                <n-select v-model:value="searchQuery.payment_status" @update:value="searchByQuery" filterable placeholder="Tất cả thanh toán" clearable :options="optionsPayment" />
              </n-form-item>
            </n-grid-item>  

            <n-grid-item span="1">
              <n-form-item label="Người tạo đơn">
                <n-select v-model:value="searchQuery.creator" @update:value="searchByQuery" filterable placeholder="Tất cả người tạo" clearable :options="creater" />
              </n-form-item>
            </n-grid-item>  

            <n-grid-item span="1">
              <n-form-item label="Ngày tạo">
                <n-date-picker v-model:value="searchQuery.range" @update:value="searchByQuery" type="daterange" clearable class="w-full" />
              </n-form-item>
            </n-grid-item>

            <n-grid-item span="3">
              <div class="flex gap-12 justify-end">
                <n-button type="success" @click="searchData">Xuất file</n-button>
    ...
    render(row) {
      return h(
        ...
        [
          ...
          h('div', { style: 'color: #888; font-size: 12px;' }, row?.customer_phone || '')
        ]
      )
    }
  },
  ...
  },
  {
    ...
    render(row) {
      return h(
        ...
        { class: 'flex gap-2' },
        [
          h(NButton, { size: 'small', onClick: () => viewOrder(row.id) }, { default: () => 'Xem' }),
          h(NButton, { size: 'small', type: 'primary', onClick: () => editOrder(row.id) }, { default: () => 'Sửa' }),
          h(NButton, { size: 'small', type: 'error', onClick: () => deleteOrder(row.id) }, { default: () => 'Xóa' })
        ]
      )
    }
  }
...
async function loadOrders() {
  try {
    ...
    if (searchQuery.value?.range && Array.isArray(searchQuery.value.range) && searchQuery.value.range.length === 2) {
      params.start_time = dayjs(searchQuery.value.range[0]).startOf('day').toISOString()
      params.end_time = dayjs(searchQuery.value.range[1]).endOf('day').toISOString()
    }
...
// Tạo hàm throttle cho loadOrders
const throttledLoadOrders = throttle(loadOrders, 500) 
