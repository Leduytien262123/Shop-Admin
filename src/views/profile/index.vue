<template>
  <AppPage show-footer>
        ...
        <div class="ml-20">
          ...
          <div class="mt-16 flex items-center">
            ...
            <span class="ml-12 opacity-60">
              Thay đổi avatar chỉ hỗ trợ liên kết trực tuyến, không cung cấp chức năng tải lên hình ảnh, nếu cần có thể tự tích hợp!
            </span>
          </div>
        </div>
      </n-space>
    ...
    <n-card class="mt-20" title="Thông tin cá nhân">
      ...
      >
        ...
        <n-descriptions-item label="Giới tính">
          {{ genders.find((item) => item.value === userStore.userInfo?.gender)?.label ?? 'Không xác định' }}
        </n-descriptions-item>
    ...
    <MeModal ref="avatarModalRef" width="420px" title="Thay đổi avatar" @ok="handleAvatarSave()">
      <n-input v-model:value="newAvatar" />
    </MeModal>

    <MeModal ref="pwdModalRef" title="Sửa mật khẩu" width="420px" @ok="handlePwdSave()">
      <n-form
        ref="pwdFormRef"
        :model="pwdForm"
        label-placement="left"
      ...
      >
        <n-form-item label="Mật khẩu cũ" path="oldPassword" :rule="required">
          <n-input v-model:value="pwdForm.oldPassword" type="password" placeholder="Vui lòng nhập mật khẩu cũ" show-password-on="mousedown" />
        </n-form-item>
        <n-form-item label="Mật khẩu mới" path="newPassword" :rule="required">
          <n-input v-model:value="pwdForm.newPassword" type="password" placeholder="Vui lòng nhập mật khẩu mới" show-password-on="mousedown" />
        </n-form-item>
      </n-form>
    </MeModal>

    <MeModal ref="profileModalRef" title="Sửa thông tin" width="420px" @ok="handleProfileSave()">
      <n-form ref="profileFormRef" :model="profileForm" label-placement="left">
        <n-form-item label="Biệt danh" path="nickName">
          <n-input v-model:value="profileForm.nickName" placeholder="Vui lòng nhập biệt danh" />
        </n-form-item>
        ...
        <n-form-item label="Địa chỉ" path="address">
          <n-input v-model:value="profileForm.address" placeholder="Vui lòng nhập địa chỉ" />
        </n-form-item>
        <n-form-item label="Email" path="email">
          <n-input v-model:value="profileForm.email" placeholder="Vui lòng nhập email" />
        </n-form-item>
      </n-form>
    </MeModal>
  </AppPage>
