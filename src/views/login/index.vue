...
function quickLogin() {
  loginInfo.value.username = 'owner'
  loginInfo.value.password = 'owner123'
  handleLogin(true)
}
...
async function handleLogin(isQuick) {
  ...
  try {
    ...
    if (loginData && loginData.success) {
      ...
      throw new Error('Không thể xử lý response từ server. Vui lòng kiểm tra console để biết chi tiết.')
    }
  }
  catch (error) {
    initCaptcha()
